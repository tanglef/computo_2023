...
Image datasets widely use crowdsourcing to collect labels, involving many workers that can annotate images for a small cost (or even freely for instance in citizen science) and faster than using expert labeling.
...
Crowdsourced datasets induce at least three major challenges to which we contribute with `peerannot`:
  ...
  - *How to score workers in a crowdsourcing experiment?* Beyond learning a classifier or inferring the true label, one might want to find a scoring or ranking between workers. For example, it is relevant for the gamification of crowdsourcing experiments [@plantgame2016]. This is treated with the `identify` command.
...
Finally, a `metadata.json` file includes all relevant information related to the crowdsourcing experiment such as the number of workers, the number of tasks, *etc.*
XXX (may be give an example for one iconic dataset)

![From collected labels to data storage in the `answers.json` file for a binary classification task on recognizing smiling faces.](./figures/json_answers.png){#fig-answers fig-align="center"}
