...
In @fig-confusionmatrix, we illustrate multiple profiles of workers simulated using `peerannot`.
In particular, we display a type of worker that can hurt data quality: the spammer.
@raykar_ranking_2011 defined a spammer as a worker that answers independently from the true label:
...
Where DS modeled workers as confusion matrices, CrowdLayer adds a layer of $\pi^{(j)}$'s into the backbone architecture as a new tensor layer to transform the output probabilities by the backbone classifier.
