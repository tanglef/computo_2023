...
While it is not very interesting to explore this dataset to compare generalization performance based on learning and aggregation strategies, it is a good benchmark for task difficulty identification and worker evaluation scoring XXX
...
To install the datasets needed, we run the `install` command from `peerannot`:
```{python}
#| echo: true
! peerannot install ./datasets/labelme/labelme.py
! peerannot install ./datasets/cifar10H/cifar10h.py
! peerannot install ./datasets/music/music.py
```
...
The music dataset is constituted of $n_{\text{task}}=100$ audio files converted as images using Mel Spectrograms. Each audio file is $30$ seconds of music from the GTZAN dataset that needs to be classified into one of the $K=10$ possible music genres.
