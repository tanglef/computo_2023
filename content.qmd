...
Crowdsourced datasets induce at least three entangled problems to which we contribute with `peerannot`:
  ...
  - *How to learn a classifier from crowdsourced datasets?* Where the first question is bound by aggregating multiple labels into a single one, this considers the case where we don't need a label to train on, but we need to train a classifier with the crowdsourced data that performs well on a testing set. This end-to-end vision, is the most common case in machine learning, however, this means that we need the actual tasks to train on -- and in crowdsourced datasets, they are not always available.
  - *How to score workers in a crowdsourcing experiment?* Beyond learning a classifier or inferring the true label, one could wish to find a ranking between workers. For example, it is relevant for the gamification of crowdsourcing experiments [@plantgame2016]
...
![From crowdsourced labels to training a classifier neural network, the full pipeline using the `peerannot` library.](./figures/strategiesbis.svg)
...
![From collected labels to data storage in the `answers.json` file for a binary classification task on recognising smiling faces.](./figures/json_answers.svg){#fig-answers fig-align="center"}
...
![Bayesian [plate notation](https://en.wikipedia.org/wiki/Plate_notation) for the DS model](./figures/bayesien_plaque_ds.svg){fig-align="center"}
...
![Bayesian [plate notation](https://en.wikipedia.org/wiki/Plate_notation) for the GLAD model](./figures/schema_bayesien_glad.svg){fig-align="center"}
...
While with this dataset comparing the implact of aggregation and end-to-end strategies might not be relevant, it is a good benchmark for task difficulty identification and worker evaluation scoring XXX
...
```{python}
#| output: false
! peerannot aggregate ./simus/exploring_reliability -s GLAD
for method in ["trace_confusion", "spam_score"]:
  ! peerannot identify ./simus/exploring_reliability/ --n-classes=5 \
                       -s {method} --labels ./simus/exploring_reliability.answers.json
```

```{python}
#| code-fold: true
#| label: fig-abilities-simu
#| fig-cap: Comparison of ability scores by workers.

path_ = Path.cwd() / "simus" / "exploring_reliability"
...
We thus add to the comparison the trace of the confusion matrices with CrowdLayer and CoNAL on the CIFAR-10H dataset.
