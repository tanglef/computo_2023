...
In practice, estimators $\hat{\pi}^{(j)}$ of the confusion matrices of each worker $w_j$ are obtained via an aggregation strategy, *e.g.,* with the Dawid and Skene's method [@dawid_maximum_1979] presented hereafter.
...
```
**XXX? je ne comprends dans la figure 6, tu utilise quelle methode? le code semble dire que tu ne fais pas de DS ou autre pour estimer le ground truth, donc utilise le ground truth? ici c'est un cas où c'est raisonnable sachant qu'on est en simulation...**

In @fig-confusionmatrix, we illustrate multiple profiles of workers, in a simulated scenario.
...
**XXX??? here you have to precise: is this just the fact that the row have constant value (i.e.,) [1/K,...1/K] or could you include the case where all rows are equal, but possibly different from the constant row? if this is the case the sentence should be : "a spammer as a worker that answers labels independently from the true label one"**
$$
...
#| code-fold: true
#| output: true
#| fig-cap: Most difficult tasks identified depending on the strategy used (entropy, GLAD or WAUM) using a Resnet34. Classes displayed are from the MV aggregation.
...
```{python}
#| code-fold: true
#| fig-cap: Most difficult tasks identified depending on the strategy used (entropy, GLAD or WAUM) using a VGG-16 with two dense layers. Classes displayed are from the MV aggregation.
classes = {
    0: "coast",
    1: "forest",
    2: "highway",
