...
$$
\mathcal{A}(x_i)=\{j\in[n_\text{worker}]: w_j \text{ answered }x_i\} \enspace.
$${#eq-workerset}
...
$$\mathcal{T}(w_j)=\{i\in[n_\text{task}]: w_j \text{ answered } x_i\}\enspace.
$${#eq-taskset}
...
![From collected labels to data storage in the `answers.json` file for a binary classification problem ($K=2$) on recognizing smiling faces. (left: how data is stored in `peerannot`, right: data collected)](./figures/json_answers.png){#fig-answers fig-align="center"}
...
- a **soft** label: $\hat y_i\in\Delta_{K}$ can represent any probability distribution (different from a Dirac distribution). In that case, each coordinate of the $K-$ dimensional vector $\hat y_i$ represents the probability to belong to the given class.
