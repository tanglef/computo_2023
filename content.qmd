...
![From crowdsourced labels to training a classifier neural network, the learning pipeline using the `peerannot` library. An optional preprocessing step using the `identify` command allows us to remove worse performing workers or images that can not be classified correctly (very bad quality for example). Then, from the cleaned dataset, if we want labels we use the `aggregate` command that follows the chosen strategy. From the aggregated labels we can train a neural network classifier with the `train` command. Otherwise, we can directly train a neural network classifier that takes into account the crowdsourcing setting in its architecture using `aggregate-deep`.](./figures/strategiesbis.png){#fig-schema fig-align="center"}

<!--
- orgiginal dataset
- screened dataset and labels
- bigger keywords
- dégager le filtre et les logos en général qui ne servent à rien
- ajouter dans la légende aggregate-deep
- train classifier + model noise
- enlever réseau et remplacer par classifier (en haut)
- mettre les database en perspective
- NN classifier (NN confusion model [autre réseau / confusion] dans une box + example in the caption de CoNAL / CrowdLayer + aggreagtion MV / DS et donner un exemple de réseau)
- Caption: dire qu'on a un preprocessing avec identify et deux choix possibles une fois qu'on a preprocess notre dataset et possibilité d'entraîner un classifier
- 1,2,3 avec les itemize (et les remettre dans le schéma)
- mettre des lettres
- mettre en texttt le code et other en other (réserver le texttt au code) et remplacer l'élément de gauche en json code le metadata
- Worker wj -> donner le j
- indice les taches et j indice les workers
- faire dans la phrase de différence maths / info avec l'indexation
 -->
...
Let us consider the classical supervised learning classification framework. A training set $\mathcal{D}=\{(x_i, y_i^\star)\}_{i=1}^{n_{\text{task}}}$ is composed of $n_{\text{task}}$ tasks $x_i\in\mathcal{X}$ (the feature space) with ground truth label $y_i^\star \in [K]={1,\dots,K}$ one of the $K$ possible classes.
...
Thus, for example for an image named `smiley_face-1`, the associated labels are stored in the `answers.json` at the key number `1`.
...
Furthermore, storing labels in a dictionary is more memory-friendly than having an array of size `(n_task,n_worker)` and writing `-1` when the worker $w_j$ did not see the task $x_i$ and $y_i^{(j)}$ otherwise (used in recent works such as @rodrigues2017learning).
...
<!-- For the first task, the feedback effort is $\vert\mathcal{A}(x_0)\vert=4$.
...
The `toy-data` example dataset is available for testing [in the `peerannot` repository](https://github.com/peerannot/peerannot/tree/main/datasets/toy-data).
...
The aggregation step can lead to two types of learnable labels $\hat y_i\in\Delta_{K}$ defined on the simplex of dimension $K-1$ depending on the use case for each task $x_i$, $i=1,\dots,n_{\text{task}}$:
...
#| fig-cap: Comparison of metrics scoring the task's intrinsic difficulty in CIFAR-10H dataset.
...
Now, let us compare the tasks's difficulty distribution depending on the strategy considered using `peerannot`.
...
We can see in @fig-difficulty-labelme that because the number of labels given per task is in $\{1,2,3\}$, the entropy only takes four values and thus does not help to dissociate the tasks.
...
def corrfunc(x, y, ax=None, **kws):
    r, _ = pearsonr(x, y)
    ax = ax or plt.gca()
    ax.annotate(rf'$\rho$ = {r:.2f}', xy=(.1, .1), xycoords=ax.transAxes)
results = {'glad': [], "entropy": []}
...
From @fig-abilities-cifar10H, we can see that in this dataset, different methods easily separate the worse workers from the rest of the crowd (workers in the tail of the distribution).
